...
describe('FileDropzone', () => {
  it('renders the drag-and-drop prompt', () => {
    ...
    expect(screen.getByText(/drag and drop your bank statement pdf/i)).toBeInTheDocument();
  });

  it('calls onFileSelected with a valid PDF file via input change', () => {
    const onFileSelected = vi.fn();
    const { container } = render(
      <FileDropzone onFileSelected={onFileSelected} error={null} />,
    );
    const input = container.querySelector('input[type="file"]') as HTMLInputElement;
    const file = new File(['%PDF-1.4 content'], 'bank.pdf', { type: 'application/pdf' });
  ...
  it('shows a file-type error and does not call onFileSelected for a non-PDF file', () => {
    const onFileSelected = vi.fn();
    const { container } = render(
      <FileDropzone onFileSelected={onFileSelected} error={null} />,
    );
  ...
  it('calls onFileSelected when a valid PDF is dropped', () => {
    const onFileSelected = vi.fn();
    render(<FileDropzone onFileSelected={onFileSelected} error={null} />);
    const dropzone = screen.getByRole('button');
    const file = new File(['%PDF-1.4 content'], 'statement.pdf', {
  ...
  it('shows a file-type error when a non-PDF file is dropped', () => {
    const onFileSelected = vi.fn();
    render(<FileDropzone onFileSelected={onFileSelected} error={null} />);
    const dropzone = screen.getByRole('button');
